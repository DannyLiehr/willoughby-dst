...
local function make_plantable(data)
	...
	local function ondeploy(inst, pt)
		local tree = SpawnPrefab("raspberrybush") 
		if tree then 
	...
	local function fn(Sim)
		...
		inst.components.inventoryitem.imagename = "raspberrybushrecipe"	
    	inst.components.inventoryitem.atlasname = "images/inventoryimages/raspberrybushrecipe.xml"
...
local plantables = 
{
	{name="raspberrybush", bank = "raspberrybush", anim="idle_dead", minspace=2},
...
for k,v in pairs(plantables) do
	...
	STRINGS.NAMES.DUG_RASPBERRYBUSH = "Raspberry Bush"

	STRINGS.CHARACTERS.GENERIC.DESCRIBE.DUG_RASPBERRYBUSH = {
	"I should probably plant it.",
}

return unpack(prefabs),
MakePlacer( "common/dug_raspberrybush_placer", "raspberrybush", "raspberrybush", "dead")
