--------------------------------------------------------------------------
--[[ Blizzards class definition ]]
--------------------------------------------------------------------------

return Class(function(self, inst)

assert(TheWorld.ismastersim, "Blizzards should not exist on client")
...
--Private
local _blizzardactive = false
local _issummer = false
local _iswet = false
local _oases = {} -- the oases are repireves from the blizzard
...
local function ToggleSandstorm()
    if _blizzardactive ~= ShouldActivateSandstorm() then
        _blizzardactive = not _blizzardactive
        inst:PushEvent("ms_blizzardchanged", _blizzardactive)
    end
end
...
-- TheWorld.components.blizzards:CalcSandstormLevel(ThePlayer)
function self:CalcSandstormLevel(ent)
    return ent ~= nil
        and ent.components.areaaware ~= nil
        and math.min(math.sqrt(CalcTaggedNodeDepthSq(ent.components.areaaware.lastpt, ent.components.areaaware.current_area, "blizzard")), TUNING.SANDSTORM_FULLY_ENTERED_DEPTH) / TUNING.SANDSTORM_FULLY_ENTERED_DEPTH
        or 0
end
...
function self:IsInSandstorm(ent)
    return _blizzardactive
        and ent.components.areaaware ~= nil
        and ent.components.areaaware:CurrentlyInTag("blizzard")
end

function self:GetSandstormLevel(ent)
    if _blizzardactive and
        ent.components.areaaware ~= nil and
        ent.components.areaaware:CurrentlyInTag("blizzard") then
        local oasislevel = self:CalcOasisLevel(ent)
        return oasislevel < 1
            and math.clamp(self:CalcSandstormLevel(ent) - oasislevel, 0, 1)
            or 0
    end
    --TODO: entities without areaaware need to know if they're inside the blizzard
    return 0
end
